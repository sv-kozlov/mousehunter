...
## Блок разрешений
В блоке разрешений указываются требования к состоянию тренажера, при соответствии которым тренировочная сессия может быть выполнена. Задаются 4 разрешения:
* targets
* vibro
* sound
* internet
...
### targets
Разрешение имеет формат `targets *`, где * - число от 1 до 16, указывающее минимальное количество мишеней, которое должно быть одновременно доступно у данной модификации тренажера для успешного проведения тренировочной сессии. Например:
```
targets 2 // у тренажера должно быть не менее двух мишеней
```
### vibro
Разрешение указывает на необходимость наличия дополнительного устройства управления вибромоторами. В случае его отсутсвия некоторые упражнения могут работать неправильно. Разрешение может принимать два значения on или off. При значении off разрешение может быть полностью пропущено. 
```
vibro on // у тренажера должны быть подключены вибромоторы
```
### sound
Разрешение указывает на необходимость наличия дополнительного устройства вывода звука. В случае его отсутствия некоторые упражнения могут работать неправильно. Разрешение может принимать два значения on или off. При значении off разрешение может быть полностью пропущено. 
```
sound on // у тренажера должны быть подключены колонки
```
### internet
Разрешение указывает на необходимость наличия доступа к сети интернет и к серверу системы. В случае его отсутствия некоторые упражнения могут работать неправильно. Разрешение может принимать два значения on или off. При значении off разрешение может быть полностью пропущено. 
```
...
```
Возможна ситуация, когда количество повторений упражнения задано достаточно большое и пользователь может захотеть перейти к следующему упражнению, не выполняя все повторения.
В таком случае в описании упражнения после идентификатора через двоеточие можно указать жест прекращения  упражнения и перехода к следующему. Например:
```
exercise MyFirstExercise: gesture clockwise { // переход к следующему  - жест вращения по ч.с.
	// тут набор команд
}
```
## Блок тренировки 
